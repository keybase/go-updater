...
func TestExecEcho(t *testing.T) {
	...
	result, err := Exec(path, []string{"echo", "arg1", "arg2"}, time.Second, testLog)
...
func TestExecNil(t *testing.T) {
	...
	_, err := execWithFunc(path, []string{"echo", "arg1", "arg2"}, execCmd, time.Second, testLog)
	require.Error(t, err)
}
...
func TestExecTimeout(t *testing.T) {
	...
	path := filepath.Join(os.Getenv("GOPATH"), "bin", "test")
	result, err := Exec(path, []string{"sleep", "10"}, timeout, testLog)
...
func TestExecBadTimeout(t *testing.T) {
	path := filepath.Join(os.Getenv("GOPATH"), "bin", "test")
	result, err := Exec(path, []string{"sleep", "1"}, -time.Second, testLog)
	assert.Equal(t, result.Stdout.String(), "")
	assert.Equal(t, result.Stderr.String(), "")
...
func TestExecForJSON(t *testing.T) {
	...
	path := filepath.Join(os.Getenv("GOPATH"), "bin", "test")
	err := ExecForJSON(path, []string{"echo", testJSON}, &testValOut, time.Second, testLog)
	assert.NoError(t, err)
...
func TestExecForJSONInvalidObject(t *testing.T) {
	...
	var testValOut testObj
	path := filepath.Join(os.Getenv("GOPATH"), "bin", "test")
	err := ExecForJSON(path, []string{"echo", validJSON}, &testValOut, time.Second, testLog)
	require.Error(t, err)
	t.Logf("Error: %s", err)
}
...
func TestExecForJSONAddingInvalidInput(t *testing.T) {
	var testValOut testObj
	path := filepath.Join(os.Getenv("GOPATH"), "bin", "test")
	err := ExecForJSON(path, []string{"echo", testJSON + "bad input"}, &testValOut, time.Second, testLog)
	assert.NoError(t, err)
	t.Logf("Out: %#v", testValOut)
	if !reflect.DeepEqual(testVal, testValOut) {
		t.Errorf("Invalid object: %#v", testValOut)
...
func TestExecForJSONTimeout(t *testing.T) {
	var testValOut testObj
	path := filepath.Join(os.Getenv("GOPATH"), "bin", "test")
	err := ExecForJSON(path, []string{"sleep", "10"}, &testValOut, 10*time.Millisecond, testLog)
...
func TestExecTimeoutProcessKilled(t *testing.T) {
	path := filepath.Join(os.Getenv("GOPATH"), "bin", "test")
	result, err := execWithFunc(path, []string{"sleep", "10"}, exec.Command, 10*time.Millisecond, testLog)
	assert.Equal(t, result.Stdout.String(), "")
	assert.Equal(t, result.Stderr.String(), "")
	assert.Error(t, err)
